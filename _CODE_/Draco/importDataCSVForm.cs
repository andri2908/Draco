...
namespace AlphaSoft
{
    public partial class importDataCSVForm : Form
    {
        ...
        private bool loadToDataGrid()
        {
            ...
            using (sr = File.OpenText(selectedFileName))
            {
        ...
        private bool saveDataTransaction()
        {
            ...
            int location = 0;
            double qtyDiff, masterProduct_Qty;

            DS.beginTransaction();

            try
            {
                location = gutil.loadlocationID(2);
                if (location == 0)
                {
                    throw new Exception("LOKASI BELUM DISET");
                }
                ...
                while (i<detailImportDataGrid.Rows.Count)
                {
                    ...
                    qtyDiff = Convert.ToDouble(productQty) - Convert.ToDouble(productOldQty);
                    ...
                    if (!productOldQty.Equals(productQty))
                    {
                        ...
                        sqlCommand = "UPDATE PRODUCT_LOCATION SET " +
                                            "PRODUCT_LOCATION_QTY = " + productQty + " " +
                                            "WHERE PRODUCT_ID = '" + productID + "' AND LOCATION_ID = " + location;
