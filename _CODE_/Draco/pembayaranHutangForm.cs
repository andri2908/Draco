...
namespace AlphaSoft
{
    public partial class pembayaranHutangForm : Form
    {
        ...
        private void loadDataDetailPayment()
        {
            ...
            sqlCommand = "SELECT PAYMENT_INVALID, PAYMENT_ID, PM_NAME AS 'TIPE', IF(PAYMENT_CONFIRMED = 1, 'Y', 'N') AS STATUS, DATE_FORMAT(PAYMENT_DUE_DATE, '%d-%M-%Y') AS 'TANGGAL', PAYMENT_NOMINAL AS 'NOMINAL', PAYMENT_DESCRIPTION AS 'DESKRIPSI' FROM PAYMENT_DEBT PC, PAYMENT_METHOD PM WHERE (PC.PM_ID - 1) = PM.PM_ID AND DEBT_ID = " + selectedDebtID;
            using (rdr = DS.getData(sqlCommand))
            {
